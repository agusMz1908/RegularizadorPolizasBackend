...
namespace RegularizadorPolizas.Infrastructure.External.VelneoAPI
{
    ...
    public async Task<IEnumerable<DepartamentoDto>> GetAllDepartamentosAsync()
    {
        ...
        {
            var tenantId = _tenantService.GetCurrentTenantId();
            _logger.LogDebug("🏛️ Getting all departamentos from Velneo API for tenant {TenantId}", tenantId);

            using var httpClient = await GetConfiguredHttpClientAsync();
            var url = await BuildVelneoUrlAsync("v1/departamentos");
            var response = await httpClient.GetAsync(url);
            response.EnsureSuccessStatusCode();
            ...
            try
            {
                var velneoResponse = await DeserializeResponseAsync<VelneoDepartamentosResponse>(response);
                if (velneoResponse?.Departamentos != null && velneoResponse.Departamentos.Any())
                {
                    var departamentos = velneoResponse.Departamentos.ToDepartamentosDtos().ToList();
                    _logger.LogInformation("✅ Successfully retrieved {Count} departamentos from Velneo API (wrapper format)",
                        departamentos.Count);
            ...
            {
                _logger.LogWarning(ex, "⚠️ Failed to parse departamentos as wrapper, trying array format");
            }

            // ✅ SEGUNDO: Si falla, intentar como array directo (fallback)
            response = await httpClient.GetAsync(url);
            response.EnsureSuccessStatusCode();
            var velneoDepartamentos = await DeserializeResponseAsync<List<VelneoDepartamento>>(response);
            if (velneoDepartamentos != null && velneoDepartamentos.Any())
            {
                var departamentos = velneoDepartamentos.ToDepartamentosDtos().ToList();
                _logger.LogInformation("✅ Successfully retrieved {Count} departamentos from Velneo API (array format)",
                    departamentos.Count);
                return departamentos;
            }

            _logger.LogWarning("⚠️ No departamentos found in Velneo API response");
            return new List<DepartamentoDto>();
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "❌ Error getting departamentos from Velneo API");
            throw new ApplicationException($"Error retrieving departamentos from Velneo API: {ex.Message}", ex);
        }
    }
